...
class ShoppingTeam(Workflow):
    ...
    def __init__(
        ...
        """
        Initializes the ShoppingTeam with agents for query enhancement, product link finding, scrapping and comparison.
        ...
        self.researcher: Agent = Agent(
            ...
            description="Collect relevant product links based on the query",
        ...
        self.scraping_agent: Agent = Agent(
            ...
            tools=[FirecrawlTools(api_key=firecrawl_api)],
    ...
    def run(self, payload: dict)-> RunResponse:
        ...
        scrapping_results = self.scraping_agent.run(search_results.content)
        logger.info(f"Scraping Results: {scrapping_results.content}")

        # Step 5: Compare products and generate HTML
        comparison_result = self.product_comparison_agent.run(search_results.content)
