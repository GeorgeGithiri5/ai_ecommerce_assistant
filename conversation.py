...
class CategoryIdentification(Agent):
    ...
    def process_query(self, query: str) -> dict:
        ...
        Returns:
            dict: A dictionary containing the identified category, required keys and message for the user.
        ...
        try:
            ...
            logger.info(
                f"Category Identification Response: {response_json}"
            )
...
class ConversationAgent(Agent):
    """
    Agent responsible for having a conversation with the user to extract
    ...
    def __init__(
        ...
        Args:
            api_key (str): Api key for the language model.
            llm_mode (str): The mode of the language model ("Groq", "OpenAI")
        """
        super().__init__(
    ...
    def __process_data(
        ...
        Args:
            Params (dict): The extracted parameters from the user's inputs.

        Returns:
            dict: A dictionary containing a user message, a query suffix, and a flag indicating
        """
        logger.info("Processing Data.....")
        missing = []
        params_keys = params.keys()
        logger.info(
            f"Extracted Parameters: {params} | Current Requirements: {self.requirements}"
        )
        ...
        if missing:
            ...
            return {
                "user_message": "Thanks you! Fetching the products..",
                "query_suffix": "",
                "missing": False,
            }
    ...
    def process_query(
        ...
        if self.step == 0:
            ...
            self.requirement_keys = self.requirement_keys()
