...
class ProductImageProcessingAgent(Agent):
    ...
    def process_image(self, image_data, user_input="") -> RunResponse:
        ...
        Returns:
            RunResponse: The response from the language model, containining the generated text query.
        """
        ...
        response: RunResponse = self.run(
            query, images=[Image(content=image_data)]
        )
        logger.info(
            f"Image processing response: {response.content}"
        )
        return response